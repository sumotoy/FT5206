...
void setup() 
{
  ...
  cts.begin(EXTRLN);//in EXTRLN mode the internal isr management it's completely bypassed! 
  tft.setTextColor(0xFFFF,0x0000);
  pinMode(CTP_INT ,INPUT);
#ifdef digitalPinToInterrupt
  attachInterrupt(digitalPinToInterrupt(CTP_INT),isr,FALLING);
#else
...
void loop() 
{
  ...
  if (_touched){
#ifdef digitalPinToInterrupt
    detachInterrupt(digitalPinToInterrupt(CTP_INT));
#else
    detachInterrupt(0);
#endif
    uint8_t i;
